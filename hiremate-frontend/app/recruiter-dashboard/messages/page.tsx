...
export default function RecruiterMessagesPage() {
  ...
  const fetchMessages = (peerId: number, reset = false) => {
    fetch(`${API_BASE_URL}/api/messages/conversations/${peerId}/messages/paginated?page=${page}&size=20`, {
    ...
    fetch(`${API_BASE_URL}/api/messages/conversations`, {
      ...
      .then((data: Conversation[]) => {
        ...
        if (receiverId && receiverUsername) {
          const existing = data.find((c: Conversation) => c.userId === receiverId);
          if (existing) {
            openChat(existing);
          } else {
            const placeholder = {
